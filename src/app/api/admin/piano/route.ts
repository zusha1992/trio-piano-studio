...
export async function POST() {
  ...
  await env.DB.prepare(
    "INSERT INTO pianos (id, brand, model, type, region, size, price_ils, color_hex, color_name_en, color_name_he, sort_order, wip, published) VALUES (?, 'New brand', '', 'upright', 'japan', '', NULL, '#1b1b1d', 'Ebony Black', 'שחור', ?, 0, 0)",
  )
    .bind(id, nextRow?.next ?? 0)
    .run();
